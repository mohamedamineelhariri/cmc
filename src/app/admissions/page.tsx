...
import { ChevronDown, ChevronUp, ExternalLink, Home, ArrowRight, GraduationCap, FileText, CreditCard, UserCheck, ClipboardList, DollarSign, Sparkles } from "lucide-react";
      ...
      <section className="bg-[#32acc1] text-white py-24 md:py-32 relative overflow-hidden">
        ...
        <div className="mx-auto px-6 lg:px-10 text-center relative" style={{ maxWidth: "var(--max-width)" }}>
          ...
          <Reveal delay={220} className="flex flex-wrap justify-center gap-4">
            <Link href="/programs" className="group inline-flex items-center gap-2 bg-white text-[#1b6f7d] hover:bg-white/90 px-7 py-4 rounded-none font-semibold text-sm transition-colors active:scale-95">
              Voir les formations <ArrowRight className="h-4 w-4 group-hover:translate-x-0.5 transition-transform" />
            </Link>
            <Link href="/contact" className="inline-flex items-center gap-2 bg-white/10 border border-white/30 text-white px-7 py-4 rounded-none font-medium text-sm hover:bg-white/20 transition active:scale-95">
              Nous contacter
            </Link>
          </Reveal>
        </div>
